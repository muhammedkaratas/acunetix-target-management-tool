    ...
    while true; do
      ...
      case "$scan_choice" in
        "yes")
          ...
          echo "Seçtiğiniz grup ID'si: $group"
          ...
          curl -s -X POST https://api.telegram.org/bot$bot_token/sendMessage -d chat_id=$chat_id -d text="Tarama başladı" > /dev/null
          break
          ;;
        "no")
          rm  target_id_list.txt
